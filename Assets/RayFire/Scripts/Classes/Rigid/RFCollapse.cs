...
namespace RayFire
{
    ...
    public class RFCollapse
    {
        ...
        public static void DamageCollapse(RayfireRigid scr, float value)
        {
            // Has no connectivity
            ...
            // Already collapsing
            if (scr.activation.cnt.collapse.inProgress == true)
                return;
                
            // Damage collapse disabled
            if (scr.activation.cnt.startCollapse != RayfireConnectivity.RFConnInitType.ByDamage)
                return;
            
            // Add damage
            scr.activation.cnt.collapseByDamageCur += value;
            
            // Start collapse if max reached
            if (scr.activation.cnt.collapseByDamageCur >= scr.activation.cnt.collapseByDamageMax)
                StartCollapse (scr.activation.cnt);
        }
