    ...
    public class RFDemolitionCluster
    {
        ...
        public static void Clusterize (RayfireRigid scr)
        {
            ...
            if (scr.tsf.childCount == 0)
            {
                ...
                RayfireMan.Log ($"{RFLog.rig_dbgn}{scr.name}{RFLog.rig_child1}", scr.gameObject);
                return;
            }
        ...
        public static void DetachEdgeShards (List<RFCluster> childClusters, List<RFShard> detachShards, int edgeShardPercentage)
        {
            if (edgeShardPercentage == 0)
                return;

            for (int i = 0; i < childClusters.Count; i++)
            {
                if (childClusters[i].shards.Count >= 5)
                {
                    int amount = childClusters[i].shards.Count * edgeShardPercentage / 100;
                    if (amount > 0)
                    {
                        // TODO detach in better way: farthest, lowest shard area, most lost neibs, ect
                        int startAmount = childClusters[i].shards.Count;
                        for (int j = childClusters[i].shards.Count - 1; j >= 0; j--)
                        {
                            ...
                            if (childClusters[i].shards[j].neibShards.Count < childClusters[i].shards[j].nAm)
                            {
                                amount--;
                                detachShards.Add (childClusters[i].shards[j]);
                                childClusters[i].shards[j].cluster = null;
                                childClusters[i].shards.RemoveAt (j);
                            }
                        }

                        // Reinit cluster
                        if (startAmount > childClusters[i].shards.Count)
                            RFShard.ReinitNeibs (childClusters[i].shards);
                    }
                }
            }
        }
        ...
        static void AddRigidComponent (RayfireRigid scr, RFShard shard, Transform parent)
        {
            ...
            // Set parent if not local and register
            RayfireMan.SetFragmentParent (shard.tm);
