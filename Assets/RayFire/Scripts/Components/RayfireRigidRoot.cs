...
namespace RayFire
{
    ...
    [AddComponentMenu (RFLog.rot_path)]
    [HelpURL (RFLog.rot_link)]
    public class RayfireRigidRoot : MonoBehaviour
    {
        ...
        public void Initialize()
        {
            ...
            {
                RayfireMan.Log (RFLog.rot_dbgn + name + RFLog.rot_noChld, gameObject);
                return;
            }
        ...
        void InitConnectivity()
        {
            ...
            if (activation.cnt != null)
            {
                ...
                if (RayfireMan.debugStateStatic == true)
                    if (cached == true && activation.cnt.cluster.cachedHost == false)
                        RayfireMan.Log (RFLog.rot_dbgn + name + RFLog.rot_Edt, gameObject);
            ...
            if (RayfireMan.debugStateStatic == true)
            {
                if (activation.con == true && activation.cnt == null)
                    RayfireMan.Log (RFLog.rot_dbgn + name + RFLog.rot_noCon, gameObject);
                if (activation.con == false && activation.cnt != null)
                    RayfireMan.Log (RFLog.rot_dbgn + name + RFLog.rot_conDis, gameObject);
            }
        }
        ...
        public void StartAllCoroutines()
        {
            ...
            if (offsetFadeShards.Count > 0)
                StartCoroutine (RFFade.FadeOffsetCor (this));
        ...
        */
        ...
        void NullCheck()
        {
            ...
            {
                RayfireMan.Log (RFLog.rot_dbgn + name + RFLog.rot_noShr, gameObject);
                ResetSetup();
            }
            ...
            {
                RayfireMan.Log (RFLog.rot_dbgn + name + RFLog.rot_noRig, gameObject);
                ResetSetup();
            }
        ...
        static bool ShardFilter(RFShard shard, RayfireRigidRoot scr)
        {
            ...
            if (shard.mf == null)
            {
                RayfireMan.Log (RFLog.rot_dbgn + shard.tm.name + RFLog.rot_noFlt, shard.tm.gameObject);
            ...
            {
                RayfireMan.Log (RFLog.rot_dbgn + shard.tm.name + RFLog.rot_noMesh, shard.tm.gameObject);
                scr.destroyShards.Add (shard);
                return false;
            }
            ...
            {
                RayfireMan.Log (RFLog.rot_dbgn + shard.tm.name + RFLog.rot_noVert, shard.tm.gameObject);
                scr.destroyShards.Add (shard);
                return false;
            }
            ...
            if (RayfireMan.colliderSizeStatic > 0)
            {
                ...
                {
                    RayfireMan.Log (RFLog.rot_dbgn + shard.tm.name + RFLog.rot_small, shard.tm.gameObject);
                    scr.destroyShards.Add (shard);
                    return false;
                }
            }
            ...
            if (scr.physics.pc == true && shard.mf.sharedMesh.vertexCount < RayfireMan.coplanarVertLimit)
            {
                ...
                {
                    RayfireMan.Log (RFLog.rot_dbgn + shard.tm.name + RFLog.rot_planar, shard.tm.gameObject);
                    scr.destroyShards.Add (shard);
                    return false;
                }
            }
