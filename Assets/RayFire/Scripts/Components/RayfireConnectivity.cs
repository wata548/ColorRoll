...
namespace RayFire
{
    ...
    [AddComponentMenu (RFLog.cnt_path)]
    [HelpURL (RFLog.cnt_link)]
    public class RayfireConnectivity : MonoBehaviour
    {
        ...
        public float             collapseByDamageCur = 0;
        public float             collapseByDamageMax = 200;
        ...
        public void Initialize()
        {
            ...
            if (meshRootHost != null)
            {
                ...
                {
                    // Has no objects
                    RayfireMan.Log (RFLog.cnt_dbgn + name + RFLog.cnt_noObj, gameObject);
                    ResetSetup();
                    return;
                }
            }
        ...
        bool WarningCheck()
        {
            ...
            if (meshRootHost != null)
            {
                ...
                {
                    RayfireMan.Log (RFLog.cnt_dbgn + name + RFLog.cnt_noRoot, gameObject);
                    return false;
                }
                ...
                {
                    RayfireMan.Log (RFLog.cnt_dbgn + name + RFLog.cnt_noCon, gameObject);
                    return false;
                }

                if (meshRootHost.simTp != SimType.Inactive && meshRootHost.simTp != SimType.Kinematic)
                {
                    RayfireMan.Log (RFLog.cnt_dbgn + name + RFLog.cnt_noAct, gameObject);
                    return false;
                }
            }
            ...
            if (rigidRootHost != null)
            {
                ...
                {
                    RayfireMan.Log (RFLog.cnt_dbgn + name + RFLog.cnt_conDis, gameObject);
                    return false;
                }
                
                if (rigidRootHost.simTp != SimType.Inactive && rigidRootHost.simTp != SimType.Kinematic)
                {
                    RayfireMan.Log (RFLog.cnt_dbgn + name + RFLog.cnt_noKin, gameObject);
                    return false;
                }
            }
        ...
        void PrepareCluster()
        {
            ...
            {
                RayfireMan.Log (RFLog.cnt_dbgn + name + RFLog.cnt_noSh, gameObject);
                cluster            = new RFCluster();
        ...
        void CheckConnectivityRigidList()
        {
            ...
            // TODO ONE NEIB DETACH FOR CHILD CLUSTERS to smooth clusters
            
            // Divide big child clusters to smaller subclusters
            SubClusters(soloShards);
        ...
        // Divide big child clusters to smaller subclusters
        void SubClusters(List<RFShard> soloShards)
        {
            // Clusterize disabled
            if (clusterize == false)
                return;

            // Subclustering disabled
            if (subEnable == false)
                return;
            
            // Has no child clusters to clusterize
            if (cluster.childClusters == null || cluster.childClusters.Count == 0)
                return;

            List<RFCluster> subClusters = new List<RFCluster>();

            // Check all clusterized clusters
            int count = cluster.childClusters.Count;
            for (int i = 0; i < count; i++)
            {
                // Skip small clusters
                if (cluster.childClusters[i].shards.Count <= subThr)
                    continue;
                
                // Set vars
                RFCluster cls             = cluster.childClusters[i];
                int       clusterAmount   = Random.Range(subMin, subMax + 1);
                int       subShardsAmount = cls.shards.Count / clusterAmount; // TODO add variation in size
                
                // Reset check state to false 
                for (int j = 0; j < cls.shards.Count; j++)
                    cls.shards[j].check = false;
                
                // Iterate through all shards
                while (cls.shards.Count > 0)
                {
                    // Collect solo shard without neibs before create new cls
                    if (SoloCheck(cls.shards[0]) == true)
                    {
                        //Debug.Log ("solo  " + cls.shards[0].tm.name);
                        soloShards.Add (cls.shards[0]);
                        cls.shards[0].cluster = null;
                        cls.shards.RemoveAt (0);
                        continue;
                    }
                    
                    // Create new sub cluster
                    RFCluster subCls = new RFCluster();
                    subCls.demolishable = cls.demolishable;
                    subCls.mainCluster  = cls.mainCluster;
                    
                    // Set uniq id after main cluster defined
                    subCls.id = RFCluster.GetUniqClusterId (subCls);
                    
                    // Add first shard to sub cluster
                    subCls.shards.Add (cls.shards[0]);
                    cls.shards[0].cluster = subCls;
                    cls.shards[0].check   = true;
                    
                    // Add neib shards while sub cluster won't be big enough
                    while (subCls.shards.Count < subShardsAmount)
                    {
                        // New cycle start amount
                        int startAmount = subCls.shards.Count;

                        // Start with first shard
                        for (int s = 0; s < subCls.shards.Count; s++)
                        {
                            // Check all neibs and add
                            for (int n = 0; n < subCls.shards[s].neibShards.Count; n++)
                            {
                                // Check if already in list. Skip if already collected.
                                if (subCls.shards[s].neibShards[n].check == true)
                                    continue;
                                
                                // Add neib shard and set collected check state
                                subCls.shards.Add (subCls.shards[s].neibShards[n]);
                                subCls.shards[s].neibShards[n].cluster = subCls;
                                subCls.shards[s].neibShards[n].check   = true;
                            }
                            
                            // Enough shards collected
                            if (subCls.shards.Count >= subShardsAmount)
                                break;
                        }
                        ...
                        // Check if new shards were not added and break while loop
                        if (startAmount == subCls.shards.Count)
                            break;
                    }
                    
                    // Collect subcluster
                    subClusters.Add (subCls);
                    
                    // Remove collected shards
                    for (int s = cls.shards.Count - 1; s >= 0; s--)
                        if (cls.shards[s].check == true)
                            cls.shards.RemoveAt (s);
                }
            }
            
            // Remove empty child clusters 
            for (int i = cluster.childClusters.Count - 1; i >= 0; i--)
                if (cluster.childClusters[i].shards.Count == 0)
                    cluster.childClusters.RemoveAt (i);
                
            // TODO remove one neib shards
            
            // Reinit neibs for subcluster shards
            for (int i = 0; i < subClusters.Count; i++)
                RFShard.ReinitNeibs (subClusters[i].shards);
            
            // Set edge debris
            if (subDeb > 0)
                RFDemolitionCluster.DetachEdgeShards (subClusters, soloShards, subDeb);
            
            // Add subclusters instead
            cluster.childClusters.AddRange (subClusters);
        }
        
        bool SoloCheck(RFShard shard)
        {
            if (shard.neibShards.Count == 0)
                return true;
            for (int i = 0; i < shard.neibShards.Count; i++)
                if (shard.neibShards[i].check == false)
                    return false;
            return true;
        }
        ...
        public void Fracture(Collider col, int debris)
        {
            ...
            else
            {
                ...
                // TODO test
                RFDemolitionCluster.DetachEdgeShards (cluster.childClusters, detachShards, debris);
        ...
        HashSet<Collider> GetOverlappedColliders (Collider col)
        {
            ...
            {
                RayfireMan.Log (RFLog.cnt_dbgn + name + RFLog.cnt_fract, gameObject);
            }
